...
export function mdToHtml(md: string): string {
  ...
  // 2. Blockquotes / Tip Boxes
  html = html.replace(/^> (.*?)$/gm, 
    '<div class="flex items-start gap-3 bg-emerald-50/50 rounded-2xl p-5 my-6 text-[14px] text-gray-700 leading-relaxed">' +
      ...
      if (index === 0 && !hasHeader) {
        ...
        cols.forEach(col => {
          ...
          if (parsedCol.includes('황제') || parsedCol.includes('왕족') || parsedCol.includes('귀족') || parsedCol.includes('양반') || parsedCol.includes('평민')) {
            parsedCol = `<span class="inline-flex items-center px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-100/50 text-xs font-semibold">${parsedCol}</span>`;
  ...
  html = html.replace(/^## (.*?)$/gm, '<h2 class="text-[16px] font-bold text-gray-800 mt-8 mb-4 flex items-center gap-2 relative before:content-[\'\'] before:inline-block before:w-1 before:h-4 before:bg-emerald-600 before:rounded-full">$1</h2>');
  ...
  const parsedLines = lines.map(line => {
    ...
    parsed = parsed.replace(/`(.*?)`/g, '<code class="bg-gray-100 text-emerald-700 px-1.5 py-0.5 rounded text-xs font-semibold font-mono">$1</code>');
...
export default function MarkdownViewer({ content }: MarkdownViewerProps) {
  // mdToHtml는 입력을 이스케이프하지 않고 그대로 HTML에 꽂아 넣으므로,
  // 이 컴포넌트가 (지금은 정적 가이드 문서지만) 향후 사용자 입력에 쓰일 경우를 대비해
  // dangerouslySetInnerHTML 직전에 항상 sanitize를 거친다.
  const htmlContent = DOMPurify.sanitize(mdToHtml(content));
