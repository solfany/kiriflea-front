...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    if (!/^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^&*]).{8,20}$/.test(password)) {
      toast.error('비밀번호: 영문+숫자+특수문자 8~20자 이내');
      return;
    }
    setLoading(true);
    try {
    ...
    <div className="min-h-screen bg-white flex flex-col items-center justify-center px-6">
      <div className="w-full max-w-sm">
        ...
        <div className="flex items-center justify-center gap-2 mb-8">
          {(['email', 'code', 'profile'] as Step[]).map((s, i) => (
            <div key={s} className="flex items-center gap-2">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${step === s ? 'bg-emerald-600 text-white' : i < (['email', 'code', 'profile'] as Step[]).indexOf(step) ? 'bg-emerald-500 text-white' : 'bg-gray-200 text-gray-500'
                }`}>
        ...
        <p className="mt-6 text-center text-sm text-gray-500">
          이미 계정이 있으신가요?{' '}
          <Link href="/login" className="text-emerald-600 font-semibold hover:underline">로그인</Link>
        </p>
      </div>
    </div>
  );
