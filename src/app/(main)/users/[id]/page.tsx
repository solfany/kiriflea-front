...
export default function UserProfilePage() {
  ...
  const { data: listings, isLoading: isListingsLoading } = useQuery<ProductListItem[]>({
  ...
  const saleCount = listings?.filter(p => p.status !== 'SOLD').length ?? 0;
  const soldCount = listings?.filter(p => p.status === 'SOLD').length ?? 0;
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-2xl p-5 mb-4 shadow-sm border border-gray-100">
        <div className="flex items-center gap-4">
          <div className="relative w-16 h-16 rounded-full bg-emerald-100 flex items-center justify-center text-2xl font-bold text-emerald-700 flex-shrink-0 overflow-hidden border border-gray-100">
            <Image
        ...
        <div className="flex gap-4 mt-4 pt-4 border-t border-gray-50">
          ...
          <div className="flex-1 text-center">
            <p className="text-lg font-bold text-emerald-700">{getMannerRank(profile.mannerScore)}</p>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 mb-4">
        <div className="px-4 pt-4">
          <h3 className="font-bold text-gray-900 text-lg">판매 상품</h3>
        </div>

        {/* 탭 */}
        <div className="flex border-b border-gray-100 mt-3 px-4">
          <button
          ...
          >
            판매중 <span className={cn('text-xs ml-0.5', activeTab === 'SALE' ? 'text-emerald-600' : 'text-gray-400')}>{saleCount}</span>
          </button>
          <button
          ...
          >
            판매완료 <span className={cn('text-xs ml-0.5', activeTab === 'SOLD' ? 'text-gray-600' : 'text-gray-400')}>{soldCount}</span>
          </button>
        </div>
